...
public class TAPCustomEditor : EditorWindow {
    ...
    private GUIContent[] _locationContents = new GUIContent[] {
        new GUIContent("X"), new GUIContent("Y")
    };

    private GUIContent[] _sizeContents = new GUIContent[] {
        new GUIContent("Width"), new GUIContent("Height")
    };
    ...
    public void OnGUI() {
            ...
            if (foldedClipDatas[i]) {
                ...
                for (var j = 0; j < data.Value.timelines.Count; ++j) {
                    ...
                    if (isClicked) {
                        ...
                        _currentSpriteTexture = data.Value.timelines[j]
                                                    .sprite.texture;
                    }
                }
        ...
        if (null != _currentSpriteTexture) {
            var textureLocation = new float[2] {
                _scrollPosition.x + 200, 100
            };

            var textureSize = 200.0f;

            EditorGUI.TextArea(new Rect(textureLocation[0], textureLocation[1] - 40, 200, EditorGUIUtility.singleLineHeight),
                               _currentKey.ToString() + " / Frame " + _currentTimelineIndex);

            EditorGUI.TextArea(new Rect(textureLocation[0], textureLocation[1] - 20, 200, EditorGUIUtility.singleLineHeight),
            ...
            if (false == preset.datas.TryGetValue(_currentKey, out var animData) ||
                0 > _currentTimelineIndex) {
            ...
            var currentRect = animData.timelines[_currentTimelineIndex]
                                      .attackCollision;

            // 위치 조정
            var location = new float[2] {
            ...
            animData.timelines[_currentTimelineIndex]
                    .attackCollision.x = location[0];

            animData.timelines[_currentTimelineIndex]
                    .attackCollision.y = location[1];
            ...
            EditorGUI.MultiFloatField(new Rect(200, textureLocation[1] + textureSize + 30 + EditorGUIUtility.singleLineHeight, 200, EditorGUIUtility.singleLineHeight),
                                      new GUIContent(), _sizeContents, size);

            animData.timelines[_currentTimelineIndex]
                    .attackCollision.width = size[0];

            animData.timelines[_currentTimelineIndex]
                    .attackCollision.height = size[1];
            ...
            if (0 < size[0] &&
                ...
                var lines = new Vector3[8] {
                    new Vector3(left_x, top_y), new Vector3(right_x, top_y), new Vector3(right_x, top_y), new Vector3(right_x, bottom_y), new Vector3(right_x, bottom_y),
